...
RandomCameraNode* ncCameraNodeList_getRandomCameraNode(int camera_node_index) {
    return (RandomCameraNode *)sNcCameraNodeList[camera_node_index].data_ptr;
}

ZoomCameraNode* ncCameraNodeList_getZoomCameraNode(int camera_node_index) {
    return (ZoomCameraNode *)sNcCameraNodeList[camera_node_index].data_ptr;
}

PivotCameraNode* ncCameraNodeList_getPivotCameraNode(int camera_node_index) {
    return (PivotCameraNode *)sNcCameraNodeList[camera_node_index].data_ptr;
}

StaticCameraNode* ncCameraNodeList_getStaticCameraNode(int camera_node_index) {
    return (StaticCameraNode *)sNcCameraNodeList[camera_node_index].data_ptr;
}
...
static void __ncCameraNodeList_setCameraNodeType(int camera_node_index, s32 type) {
    ...
    switch(sNcCameraNodeList[camera_node_index].type) {
        case NC_CAMERA_NODE_TYPE_4_RANDOM:
            cameraNodeType4_free((RandomCameraNode *)sNcCameraNodeList[camera_node_index].data_ptr);
            break;
        case NC_CAMERA_NODE_TYPE_3_ZOOM:
            cameraNodeType3_free((ZoomCameraNode *)sNcCameraNodeList[camera_node_index].data_ptr);
            break;
        case NC_CAMERA_NODE_TYPE_1_PIVOT:
            cameraNodeType1_free((PivotCameraNode *)sNcCameraNodeList[camera_node_index].data_ptr);
            break;
        case NC_CAMERA_NODE_TYPE_2_STATIC:
            cameraNodeType2_free((StaticCameraNode *)sNcCameraNodeList[camera_node_index].data_ptr);
            break;
    ...
    {
        case NC_CAMERA_NODE_TYPE_4_RANDOM:
            sNcCameraNodeList[camera_node_index].data_ptr = (uintptr_t)cameraNodeType4_init();
            break;
        case NC_CAMERA_NODE_TYPE_3_ZOOM:
            sNcCameraNodeList[camera_node_index].data_ptr = (uintptr_t)cameraNodeType3_init();
            break;
        case NC_CAMERA_NODE_TYPE_1_PIVOT:
            sNcCameraNodeList[camera_node_index].data_ptr = (uintptr_t)cameraNodeType1_init();
            break;
        case NC_CAMERA_NODE_TYPE_2_STATIC:
            sNcCameraNodeList[camera_node_index].data_ptr = (uintptr_t)cameraNodeType2_init();
            break;
        case NC_CAMERA_NODE_TYPE_0_UNK:
            break;
    }
    ...
    for(i = 0; i < NC_CAMERA_NODE_LIST_CAPACITY; i++) {
        if(sNcCameraNodeList[i].valid) {
            switch(sNcCameraNodeList[i].type) {
                case NC_CAMERA_NODE_TYPE_4_RANDOM:
                    sNcCameraNodeList[i].data_ptr = (uintptr_t)defrag((void *)sNcCameraNodeList[i].data_ptr);
                    break;
                case NC_CAMERA_NODE_TYPE_3_ZOOM:
                    sNcCameraNodeList[i].data_ptr = (uintptr_t)defrag((void *)sNcCameraNodeList[i].data_ptr);
                    break;
