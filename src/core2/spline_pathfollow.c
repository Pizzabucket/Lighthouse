...
// [port] Glspline bitfield structs use BE bit ordering (first member = MSB).
...
typedef struct{
    ...
    struct {
        u32 _pad_bit0: 1; // [port] explicit padding for implicit BE bit 0 (31 declared bits in original)
        u32 pad_bit3:3;
        u32 bit4: 1;
        u32 bit5: 1;
        u32 bit6: 1;
...
static void glspline_convert_from_nodeprop(Union_glspline *dst) {
    u8 *p = (u8 *)dst;

    // Word 0 (offset 0-3): Two LE s16 (x, y) → BE u32 for f32 interpretation
    u16 hw0 = *(u16 *)(p + 0);
    u16 hw1 = *(u16 *)(p + 2);
    union { f32 f; u32 u; } conv;
    conv.u = ((u32)hw0 << 16) | hw1;
    *(f32 *)(p + 0) = conv.f;

    // Word 1 (offset 4-7): LE s16 (z) + LE u16 (NodeProp bitfield: radius:9|bit6:6|bit0:1)
    u16 z_raw = *(u16 *)(p + 4);
    u16 field6_le = *(u16 *)(p + 6);
    u16 radius = field6_le & 0x1FF;
    u16 np_bit6 = (field6_le >> 9) & 0x3F;
    u16 np_bit0 = (field6_le >> 15) & 1;
    u16 field6_be = (radius << 7) | (np_bit6 << 1) | np_bit0;
    *(u32 *)(p + 4) = ((u32)z_raw << 16) | field6_be;

    // Word 2 (offset 8-11): LE u16 (unk8) + u8 (unkA) + u8 (padB) → BE u32
    u16 unk8_val = *(u16 *)(p + 8);
    u8 unkA = p[10];
    u8 padB = p[11];
    *(u32 *)(p + 8) = ((u32)unk8_val << 16) | ((u32)unkA << 8) | padB;

    // Word 3 (offset 12-15): LE u32 (NodeProp: yaw:9|scale:23) → BE u32
    u32 field12_le = *(u32 *)(p + 12);
    u32 yaw = field12_le & 0x1FF;
    u32 scale = (field12_le >> 9) & 0x7FFFFF;
    *(u32 *)(p + 12) = (yaw << 23) | scale;

    // Word 4 (offset 16-19): LE u32 (NodeProp: unk10_31:12|unk10_19:12|pad10_7:1|unk10_6:1|pad10_5:4|unk10_0:2) → BE u32
    u32 field16_le = *(u32 *)(p + 16);
    u32 f_unk10_31 = field16_le & 0xFFF;
    u32 f_unk10_19 = (field16_le >> 12) & 0xFFF;
    u32 f_pad10_7 = (field16_le >> 24) & 1;
    u32 f_unk10_6 = (field16_le >> 25) & 1;
    u32 f_pad10_5 = (field16_le >> 26) & 0xF;
...
s32 func_80340760(s32 arg0, s32 *arg1, f32 *arg2, f32 arg3[3], s32 arg4, f32 *arg5, f32 *arg6) {
...
s32 func_80343694(Actor *actor, s32 indx, s32 begin, s32 end, s32 count, s32 stride) {
    ...
    s32 var_v1; // [port] Must be s32, not bool — func_803422D4 returns 0/1/2, and bool truncates 2→1
    f32 sp48;
    Union_glspline *i_ptr;
