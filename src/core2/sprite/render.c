...
BKSpriteDisplayData * func_80344A1C(BKSprite *arg0){
    ...
    for(i = 0; i < arg0->frameCnt; i++){//L80344A88
        s1 = ALIGN(s1, 0x10);
        // [port] 0x12C0 was N64 scratch size for 200 Vtx (16B) + 200 Gfx (8B).
        // On 64-bit, Gfx is 16B, so compute dynamically.
        s6 = (BKSpriteDisplayData *)bk_realloc(s6, header_size + s1 + 200 * sizeof(Vtx) + 200 * sizeof(Gfx));
        vtx_start = (Vtx *)((uintptr_t)s6 + header_size + s1);
    ...
    for(i = 0; i < arg0->frameCnt; i++, v1++){//L80344B94
        v1->vtx = (Vtx *)(s1 + ((uintptr_t)s6 + header_size));
        v1->gfx = (Gfx *)((uintptr_t)v1->vtx + frame_vtx_size[i]);
