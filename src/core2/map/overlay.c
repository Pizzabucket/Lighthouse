...
/* .data */
static struct level_overlay_map_s sLevelToOverlayMap[] = {
...
enum overlay_e leveloverlay_getOverlayFromLevel(enum level_e lvl) {
    int i;
    for (i = 0; sLevelToOverlayMap[i].level_id; i++) {
        if (sLevelToOverlayMap[i].level_id == lvl)
            return sLevelToOverlayMap[i].overlay_id;
    }
    return OVERLAY_0_CORE2;
}
