...
bool AnimTextureListCache_tryGetTextureOffset(s32 index, s32 texture_index, s32 *texture_offset) {
    ...
    if (item->anim_texture_info[texture_index].frame_size != 0) {
        s32 frame = (s32)item->current_frame[texture_index];
        if (frame < 0) frame = 0;
        if (frame >= item->anim_texture_info[texture_index].frame_cnt)
            frame = item->anim_texture_info[texture_index].frame_cnt - 1;
        *texture_offset = frame * item->anim_texture_info[texture_index].frame_size;
        return true;
    }
    return false;
}
