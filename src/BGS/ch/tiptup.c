...
void chTiptup_setState(Actor *this, s32 arg1){
    ...
    if(this->state == 3){
        ...
        for(; i < D_80390864[unqPtr->unkA]; i++){
            ...
            timedFunc_set_2(tmpf, (GenFunction_2) chTiptup_activatePattern, (uintptr_t)this->marker, rand2);
            timedFunc_set_2(tmpf + 0.1, (GenFunction_2)chTiptup_activatePattern, (uintptr_t)this->marker, 0);
            prev_member = rand2;
        }//L80388B7C
        ...
        if(volatileFlag_get(VOLATILE_FLAG_2_FF_IN_MINIGAME)){
            timedFunc_set_1(sp54, (GenFunction_1)chTiptup_activateTimer, (uintptr_t)this->marker);
        }else{
            this->state = 0x05;
        }
    ...
    if(this->state == 4){
        ...
        for(j = 0; j < bk_vector_size(unqPtr->unk4); j++){
            ...
            tmpf += randf2(1.0f, 1.5f);
            timedFunc_set_2(tmpf, (GenFunction_2)chTiptup_activatePattern, (uintptr_t)this->marker, s1->unk0);
            timedFunc_set_2(tmpf + 0.1, (GenFunction_2)chTiptup_activatePattern, (uintptr_t)this->marker, 0);
        }
        timed_exitStaticCamera(tmpf += 2.5);
...
void chTiptup_choirHitReaction(ActorMarker *this, s32 arg1){
    ...
    if((s32)unqPtr->unk0 >= (s32) bk_vector_size(unqPtr->unk4)){
        ...
        if(arg1 == tmp->unk0){
            timedFunc_set_1(0.5f, (GenFunction_1)chTiptup_correctHit, (uintptr_t)thisActor->marker);
        }
        else{
